 ...
 *	背包管理器
 */
#ifndef store_manager_h__
#define store_manager_h__
...
public:
    StoreManager(Player* player);
    ~StoreManager();

    void    Clear();

    void    LoadstoreInfo(const ITEM_INFO& info);
    void    FillStoreInfo(ITEM_INFO& info);
...
private:
    void OnItemInfoReq(void* data);             // 获取物品信息
