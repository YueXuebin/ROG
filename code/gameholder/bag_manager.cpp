...
ItemManager* BagManager::GetItemMgr()
{
    return m_player->GetItemManager();
}

BagManager::BagManager(Player* player):
m_player(player)
{
    Clear();

    for (uint32 i = 0; i < ITEM_BAG_MAX_GRID_NUM; ++i)
    {
...
// 加载背包信息
// ==================================================
void BagManager::LoadBagInfo(const ITEM_INFO& info)
{
    ItemBase* pItem(NULL);
...
// 填充背包信息
// ==================================================
void BagManager::FillBagInfo(ITEM_INFO& info)
{
    info.Clear();
...
void BagManager::OnRecv(uint32 msgID, void* data)
{
    switch(msgID)
    {
    case MSG_BAG_INFO_REQ:
        {
            OnItemInfoReq(data);
        }
        break;
    case MSG_BAG_SORT_REQ:
        {
            OnItemSortReq(data);
        }
        break;
...
uint16 BagManager::AddItem(uint32 reason, ItemBase* pItem, int32 pos)
{
    if (!pItem)
    {
        CnAssert(false);
    ...
    if (NULL_GRID == pos)
    {
        ...
        {
            for (size_t i=0; i<ITEM_BAG_MAX_GRID_NUM; ++i)
            {
    ...
    if (pDst)
    {
        // 加进来的不是背包内的东西，终点就不应该有东西
        if (pItem->GetPlace() != PLACE_BAG && pItem->GetPlace() != PLACE_STORE)
        {
            CnAssert(false);
            return ERR_FAILED;
        }
    ...
    pItem->SetPos(pos);
    AddToGrid(pItem);
    if (PLACE_BAG != nPlace && PLACE_STORE!=nPlace)
    {
...
// 删除物品
// ==================================================
void BagManager::SubItem(uint32 reason, int32 pos, int32 num)
{
    // 不存在的格子
    if (!IsValidGrid(pos))
    {
...
int32 BagManager::GetEmptyGridNum() const
{
    int32 num(0);
...
int32 BagManager::FindEmptyGrid() const
{
    for (uint32 i = 0; i < GetItemGrid().size(); ++i)
    {
        if (!GetItemGrid()[i])
...
bool BagManager::IsEmptyGrid( int32 pos ) const
{
    if (!IsValidGrid(pos))
    {
        CnAssert(false);
...
bool BagManager::IsValidGrid(int32 pos) const
{
    return (pos >= 0 && pos < ITEM_BAG_MAX_GRID_NUM);
}

bool BagManager::IsCanStack(ItemBase * pSrc, ItemBase * pDst) const
{
    if (!pSrc || !pDst)
    {
        CnAssert(false);
...
ItemBase* BagManager::GetItemByPos( int32 pos )
{
    if (!IsValidGrid(pos))
    {
        CnAssert(false);
...
ItemBase* BagManager::GetItemById(int32 id)
{
    ItemBase* pTemp(NULL);
    for (size_t i = 0; i < ITEM_BAG_MAX_GRID_NUM; ++i)
    {
...
void BagManager::GetItemListById(int32 id, std::vector<ItemBase*>& outList)
{
    outList.clear();
...
void BagManager::AddToGrid( ItemBase * pItem, bool bSwap )
{
    CnAssert(pItem);
...
void BagManager::StackItem(ItemBase * pSrc, ItemBase * pDst)
{
    if (!pSrc || !pDst)
    {
        CnAssert(false);
    ...
    if (pSrc->GetPlace() != PLACE_BAG&& pSrc->GetPlace() != PLACE_STORE)
    {
        GetItemMgr()->NtfOptItemAdd(pDst, nMove);
    }
}

uint16 BagManager::SwapItem(uint32 srcPlace, int32 srcPos, uint32 dstPlace, int32 dstPos)
{
...
uint16 BagManager::SellItem(int32 pos)
{
    if (!IsValidGrid(pos))
    {
        CnAssert(false);
...
bool BagManager::SortItem(ItemBase* pItemA, ItemBase* pItemB)
{
    if (!pItemA && !pItemB)
    {
        return false;
...
void BagManager::OnItemInfoReq(void* data)
{
    SendItemInfo();
}

void BagManager::OnItemSortReq(void* data)
{
    ItemBase * pTemp(NULL);
    for (size_t i=0; i<ITEM_BAG_MAX_GRID_NUM; ++i)
    {
        for (size_t j=i+1; j<ITEM_BAG_MAX_GRID_NUM; ++j)
        {
...
void BagManager::SendItemInfo()
{
    ITEM_INFO itemInfo;
    FillBagInfo(itemInfo);
    m_player->SendToGate(MSG_BAG_INFO_ACK, &itemInfo);
}