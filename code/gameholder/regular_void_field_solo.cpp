...
void VoidFieldSoloPlayerRecord::AddChestsNum(int32 val)
{
    int64 chests_num = 0;
    CnAssert(val >= 0);
...
void VoidFieldSoloPlayerRecord::SubChestsNum(int32 val)
{
    CnAssert(val >= 0);
    if(val < 0) return;
    if(m_chests_num > val)
...
RegularVoidFieldSolo::RegularVoidFieldSolo(RegularDoc* pDoc) :
RegularWorld(pDoc)
{
    // 初始化类型条件表       // m_openCondList的id就是规则类型
    VoidFieldSoloOpenConditon cdRecord;
    // 普通
    m_openCondList.push_back(cdRecord);
    // 珍兽
    cdRecord.m_start_time = 0;
    cdRecord.m_run_time = VoidFieldSoloConfig::Instance()->GetRegularBossRunTime();
    m_openCondList.push_back(cdRecord);
...
RegularVoidFieldSolo::~RegularVoidFieldSolo()
{

}

void RegularVoidFieldSolo::OneSecondUpdate( int32 deltatime )
{
...
void RegularVoidFieldSolo::OnCreatureEnter( BattleUnit* creature )
{
    if(!creature)
    {
        CnAssert(false);
...
void RegularVoidFieldSolo::OnCreatureLeave( BattleUnit* creature )
{
    if(!creature)
        return;
    ...
    VoidFieldSoloPlayerRecord* pRecord = GetRecord(player);
    CnAssert(!pRecord || pRecord->GetChestsNum() == 0);         //  确保玩家退出时，虚空宝箱数为0
}

void RegularVoidFieldSolo::CalIntoTime(Player* player)
{
    if(!player)
        return;
...
void RegularVoidFieldSolo::ReceiveEvent(uint32 frameCount, uint32 eventId, int32 arg1, int32 arg2, int32 arg3)
{
...
void RegularVoidFieldSolo::Update(uint32 curr_tick)
{
    RegularWorld::Update(curr_tick);
}

bool RegularVoidFieldSolo::IsEnemy(BattleUnit* attacker, BattleUnit* defencer)
{
...
// 设置规则的场景
// ==================================================
void RegularVoidFieldSolo::SetScene(Scene* pScene)
{
    RegularWorld::SetScene(pScene);
}

void RegularVoidFieldSolo::OnStart()
{
...
// 填充开始位置
// ==================================================
void RegularVoidFieldSolo::FillStartPostion(Player* player, uint32 triggerID)
{
    Trigger* trigger = 0;
...
// 获得当前类型
// ==================================================
int32 RegularVoidFieldSolo::GetCurrType()
{
    return m_currType;
}
...
// 设置当前类型
// ==================================================
void RegularVoidFieldSolo::SetCurrType(int32 mType)
{
    ...
    if((0 <= mType) && (mType < VOID_FIELD_SOLO_TYPE_MAX))
        m_currType = mType;
...
// 切换规则
// ==================================================
void RegularVoidFieldSolo::UpdateRegularState()
{
    // 检查运行
    if (m_currType != VOID_FIELD_TYPE_NORMAL)
    {
...
void RegularVoidFieldSolo::GotoNormalState()
{
    // 如果是本轮的最后一个频道
    if (VoidFieldSoloManager::Instance()->IsLastInChannelList(m_currType, m_pScene->GetChannelID()) == true)
    {
        // 设置open time
        VoidFieldSoloManager::Instance()->SetOpenTime(m_currType);
        // 清空列表
        VoidFieldSoloManager::Instance()->ClearChannelList(m_currType);
    }
...
void RegularVoidFieldSolo::SetStateStartTime(int32 mType)
{
    uint32 now = (uint32)Crown::SDNow().GetTimeValue();
...
// 通知角色宝箱数量
// ==================================================
void RegularVoidFieldSolo::SendVoidFieldChestsNum(Player* player, int32 num)
{
    ...
    player->SendToGate(MSG_VOID_FIELD_SOLO_CHESTS_NUM_NTF, &sendMsg);
}

// ==================================================
// 抢夺宝箱
// ==================================================
void RegularVoidFieldSolo::RobChests(uint32 deadID, uint32 killerID, int32 rate)
{
    Player* deadPlayer = NULL;
    Player* killPlayer = NULL;
    VoidFieldSoloPlayerRecord* deadRecord = NULL;
    VoidFieldSoloPlayerRecord* killRecord = NULL;
    int64 rob_num = 0;
...
void RegularVoidFieldSolo::ResetAddExpTime()
{
    int32 interval_time = VoidFieldSoloConfig::Instance()->GetRewardExpInterval(m_currType);
    uint32 now = (uint32)Crown::SDNow().GetTimeValue();
...
void RegularVoidFieldSolo::TriggerTaskEvent(uint32 taskEventId)
{
    for (PlayerRecordMap::iterator iter = m_recordMap.begin(); iter != m_recordMap.end(); ++iter)
    {
...
void RegularVoidFieldSolo::PushMessage()
{
    for (PlayerRecordMap::iterator iter = m_recordMap.begin(); iter != m_recordMap.end(); ++iter)
    {
        Player* player = PlayerManager::Instance()->GetPlayerByPlayerID(iter->second.GetPlayerID());
...
void RegularVoidFieldSolo::RewardPlayerExp()
{
    int32 interval_time = VoidFieldSoloConfig::Instance()->GetRewardExpInterval(m_currType);
    int32 exp_value     = VoidFieldSoloConfig::Instance()->GetRewardExpNum(m_currType);
    uint32 now = (uint32)Crown::SDNow().GetTimeValue();
...
// 通知当前副本规则类型
// ==================================================
void RegularVoidFieldSolo::AddPlayerChests(uint32 creatureID)
{
...
// 添加虚空宝箱数量
// ==================================================
void RegularVoidFieldSolo::AddChestsNum(Player* player)
{
...
void RegularVoidFieldSolo::AddDropItem(Player* player)
{
    if(!player)
        return;
    ...
    REWARD_ITEM* rewardItem = VoidFieldSoloConfig::Instance()->GetDropRandItem(randNum);
    if(rewardItem)
    {
...
bool RegularVoidFieldSolo::CanMonsterDrop(int32 diff_level)
{
    int32 rate = VoidFieldSoloConfig::Instance()->GetDiffLevelDropRate(diff_level);
    int32 rand_num = Crown::SDRandom(BASE_RATE_NUM);
    return rand_num < rate;
}

bool RegularVoidFieldSolo::CanStateDrop()
{
    int32 rate = 0;
    int32 rand_num = 0;
    ...
    case VOID_FIELD_TYPE_NORMAL:
        rate = VoidFieldSoloConfig::Instance()->GetNormalChestsRate();
        break;
    case VOID_FIELD_TYPE_BOSS:
        rate = VoidFieldSoloConfig::Instance()->GetBossChestsRate();
        break;
    default:
        break;
    }
...
//虚空宝箱的掉落概率
bool RegularVoidFieldSolo::CanChestsProbability(Player* player,int32 lower)
{
...
// 设置玩家的战斗状态
// ==================================================
void RegularVoidFieldSolo::SetPlayersFightMode()
{
...
// 通知当前副本规则类型
// ==================================================
void RegularVoidFieldSolo::NotifyRegularType(Player* player)
{
    ...
    if(player)
    {
        player->SendToGate(MSG_VOID_FIELD_SOLO_REGULAR_TYPE_NTF, &sendMsg);
    }
    else
    {
        m_pScene->NotifyMessageToAll(MSG_VOID_FIELD_SOLO_REGULAR_TYPE_NTF, &sendMsg);
    }
}
...
// 战斗攻击事件
// ==================================================
void RegularVoidFieldSolo::OnBattleEventAttack(uint32 attackerID, uint32 defencerID)
{
    Player* attackPlayer = 0;
    Player* defencePlayer = 0;
...
// 接收角色死亡事件
// ==================================================
void RegularVoidFieldSolo::OnBattleEventKill(uint32 attackerID, uint32 defencerID)
{
    Player* attackPlayer = 0;
    Player* defencePlayer = 0;
    EnemyManager* attackEnemyMgr = 0;
...
void RegularVoidFieldSolo::OnBattleEventMonsterDead(uint32 monsterID, uint32 killerID)
{
...
//         {
//             //宝箱掉落概率的计算
//             if(player->GetVoidFieldTime() <= GameSettingManager::Instance()->GetVoidFieldSoloDropChestTime())
//             {
//                 if(CanStateDrop())
//                 {
...
//             else
//             {
//                 std::map<uint32, int32> curmap = VoidFieldSoloConfig::Instance()->GetChestsProbabilityByTime();
//                 std::map<uint32, int32>::iterator iter = curmap.begin();
// 
//                 if(iter != curmap.end())
//                 {
//                     int32  time = player->GetVoidFieldTime() - GameSettingManager::Instance()->GetVoidFieldSoloDropChestTime();
...
// 获得记录
// ==================================================
VoidFieldSoloPlayerRecord* RegularVoidFieldSolo::GetRecord(Player* player)
{
    PlayerRecordMap::iterator playerIt;
...
Player* RegularVoidFieldSolo::GetPlayerByUnitID(uint32 uintID)
{
...
void RegularVoidFieldSolo::KillAllMonsters()
{
...
void RegularVoidFieldSolo::StopRefreshMonsters()
{
    // 停止刷怪
...
void RegularVoidFieldSolo::RecoverRefreshMonsters()
{
    Scene::SpawnMap& spawnMap = GetScene()->GetSpawn();
    for(Scene::SpawnMap::iterator spawnIt = spawnMap.begin(); spawnIt != spawnMap.end(); ++spawnIt)
    {
...
// 添加记录
// ==================================================
void RegularVoidFieldSolo::AddRecord(Player* player)
{
    int32 interval_time = VoidFieldSoloConfig::Instance()->GetRewardExpInterval(m_currType);
    uint32 now = (uint32)Crown::SDNow().GetTimeValue();

    PlayerRecordMap::iterator playerIt;
    VoidFieldSoloPlayerRecord record;
...
// 删除记录
// ==================================================
void RegularVoidFieldSolo::DelRecord(Player* player)
{
    PlayerRecordMap::iterator playerIt;
    ...
    if(playerIt != m_recordMap.end())
    {
        //m_recordMap.erase(playerIt);  // 不再真正删除该玩家，只是删除该玩家的某些记录，但保留该玩家是否参加了当轮的珍兽标志
...
// 结算宝箱
// ==================================================
void RegularVoidFieldSolo::SettleChests(Player* player)
{
    VoidFieldSoloPlayerRecord* record = NULL;
    REWARD_TABLE rewardTable;
    ...
    for(int32 i=0; i<record->GetChestsNum(); ++i)
    {
        ...
        rewardItem = VoidFieldSoloConfig::Instance()->GetChestsRandItem(randNum);
        if(rewardItem)
        {
            MergeRewardTableItemList(rewardTable, *rewardItem);
        }
...
// 记录虚空幻境击杀信息
// ==================================================
void RegularVoidFieldSolo::AddPersonalKillInfo(Player* attacker, Player* defencer)
{
...
// 添加虚空幻境经验信息
// ==================================================
void RegularVoidFieldSolo::AddPersonalExpInfo(Player* player, VOID_FIELD_PERSONAL_INFO_NTF* info, int32 exp)
{
...
void RegularVoidFieldSolo::AddPersonalDropItemInfo(Player* player, VOID_FIELD_PERSONAL_INFO_NTF* info, REWARD_ITEM* drop_item)
{
...
// 发送虚空幻境信息
// ==================================================
void RegularVoidFieldSolo::SendVoidFieldInfoNtf(Player* player, VOID_FIELD_PERSONAL_INFO_NTF* info)
{
    if(!player || !info)
        return;

    // 发送玩家信息
    player->SendToGate(MSG_VOID_FIELD_SOLO_PERSONAL_INFO_NTF, info);
...
// 接收复活请求
// ==================================================
void RegularVoidFieldSolo::OnPlayerResurgenceReq(int32 arg1)
{
...
// 计算TriggerID
// ==================================================
uint32 RegularVoidFieldSolo::CalPlayerTriggerID(Player* player)
{
    uint32 triggerID = 0;
...
// 发送复活询问
// ==================================================
void RegularVoidFieldSolo::SendResurgenceAsk(Player* player)
{
    ...
    sendMsg.count_down = VoidFieldSoloConfig::Instance()->GetResurgenceTime();
...
// 广播频道开启类型
// ==================================================
void RegularVoidFieldSolo::SendRegularTypeMarquee(int32 regularType, uint32 channelNum)
{
    ...
    // 根据副本类型获得语言包
	if (regularType == VOID_FIELD_TYPE_BOSS)
    {
        langID = VoidFieldSoloConfig::Instance()->GetBossBroadcastID();

    	// 组织广播语言格式
    	sendMsg.parame_list.push_back(MarQueeVoidFieldSoloRegularType(langID, channelNum));

    	// 广播数据
    	GateHandler::Instance()->SendToAll(MSG_SERVER_MARQUEE, &sendMsg, BROADCAST_PLAYER_ID);
	}
}

void RegularVoidFieldSolo::CheckPlayerEvilState(Player* player)
{
    if (!player)
    {
        return;
...
void RegularVoidFieldSolo::AddPlayerRedNameBuff(Player* player)
{
    if (!player)
    {
        return;
...
void RegularVoidFieldSolo::ClearPlayerRedNameBuff(Player* player)
{
    if (!player)
    {
        return;
...
void RegularVoidFieldSolo::ClearBossData()
{
...
// 创建 BOSS
// ==================================================
void RegularVoidFieldSolo::CreateBoss()
{
    ...
    // 检查是否有这个 BOSS
    m_bossID = VoidFieldSoloConfig::Instance()->GetRareBossID();
    CnAssert(m_bossID);
...
void RegularVoidFieldSolo::StrongBoss(Creature* battleUnit)
{
}
...
// 接收角色死亡事件
// ==================================================
void RegularVoidFieldSolo::OnBossModeBattleEventKill(uint32 attackerID, uint32 defencerID)
{
    ...
    val = damageInfo->damage * VoidFieldSoloConfig::Instance()->GetBossPlayerDeadRate();
    damageInfo->damage = (uint32)(val / BASE_RATE_NUM);
}

void RegularVoidFieldSolo::OnDamage(uint32 attackerID, int32 damage)
{
...
void RegularVoidFieldSolo::UpdateDamage()
{
    ...
    for(Scene::PlayerMap::iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        if(pInfo)
        {
            ...
            it->second->SendToGate(MSG_VOID_FIELD_SOLO_BOSS_DAMAGE_NTF, &sendMsg);
        }
        else
        {
            it->second->SendToGate(MSG_VOID_FIELD_SOLO_BOSS_DAMAGE_NTF, &topDamageList);
        }
    }
}

void RegularVoidFieldSolo::OnDead(uint32 attackerID)
{
    if(attackerID)
    {
        // 计算奖励
...
void RegularVoidFieldSolo::BossLevelUp()
{
    Crown::CSDDateTime now = Crown::SDNow();

    // TODO:
    WorldBossManager::Instance()->SetBoss(VOID_FIELD_SOLO_BOSS_SAVE_TYPE_ID, m_bossID , (uint32)now.GetTimeValue());
    WorldBossManager::Instance()->LevelUpBoss(m_bossID , m_startcreatebooss, (uint64)(VoidFieldSoloConfig::Instance()->GetBossKilledLevelupSeconds()));
...
// 按由大到小排序
// ==================================================
bool RegularVoidFieldSolo::SortDamage(VoidFieldBossDamageInfo* a, VoidFieldBossDamageInfo* b)
{
...
// 在列表中查找同名角色数据
// ==================================================
RegularVoidFieldSolo::VoidFieldBossDamageInfo* RegularVoidFieldSolo::GetNotifyDamageInfo(WORLD_BOSS_DAMAGE_LIST& list, Player* player)
{
    DamageMap::iterator damageIt;
...
// 获得角色的伤害值
// ==================================================
RegularVoidFieldSolo::VoidFieldBossDamageInfo* RegularVoidFieldSolo::GetBossDamageInfo(uint64 playerID)
{
...
// 检查每个玩家是否超出攻击范围
// ==================================================
void RegularVoidFieldSolo::CheckOverRange()
{
    NONE sendMsg;
    DamageMap::iterator damageIt;
    int64 num = 0;
    ...
    for(Scene::PlayerMap::iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        if (subTime == 0)
        {
            m_subScopeMap[it->second->GetPlayerID()] = now + VoidFieldSoloConfig::Instance()->GetBossOverRangeTime();
            it->second->SendToGate(MSG_VOID_FIELD_SOLO_OVER_RANGE_NTF, &sendMsg);
        }
        else if (subTime > now && (subTime - now) % VoidFieldSoloConfig::Instance()->GetOutScopeInterval() == 0)
        {
            it->second->SendToGate(MSG_VOID_FIELD_SOLO_OVER_RANGE_NTF, &sendMsg);
        }
        ...
        if (0 < subTime && subTime <= now) 
        {
            ...
            {
                num = damageIt->second.damage * VoidFieldSoloConfig::Instance()->GetBossOverRangeRate();
                num /= BASE_RATE_NUM;
...
// 结算奖励
// ==================================================
void RegularVoidFieldSolo::SettleReward()
{
    VoidFieldBossDamageInfo* damageInfo = NULL;
...
// 排名第一奖励
// ==================================================
void RegularVoidFieldSolo::SendTopOneRewardMail(uint32 centerID, uint64 playerID)
{
    if(!playerID)
        return;

    REWARD_TABLE& rewardTable = VoidFieldSoloConfig::Instance()->GetBossTopOneRewards();
    /*
...
// 排名第一公会奖励
// ==================================================
void RegularVoidFieldSolo::SendTopOneGuildRewardMail(uint64 guildID)
{
    uint64 playerID = 0;
    ...
    REWARD_TABLE& rewardTable = VoidFieldSoloConfig::Instance()->GetBossTopOneGuildRewards();
...
// 最后击杀者奖励
// ==================================================
void RegularVoidFieldSolo::SendKillerRewardMail(uint32 uintID)
{
    ...
    REWARD_TABLE& rewardTable = VoidFieldSoloConfig::Instance()->GetBossKillerRewards();
    /*
    MailManager::SendMail(player->GetCenterSvrID(), player->GetPlayerID(), "", 
...
uint32 RegularVoidFieldSolo::GetPlayerSubScoreTime(uint64 playerID)
{
...
void RegularVoidFieldSolo::BossKillProcess(uint32 uintID)
{
    Player* player = m_pScene->GetPlayerByObjectId(uintID);
...
void RegularVoidFieldSolo::BossEscapeProcess()
{
    // 公告珍兽boss逃跑
    SERVER_MARQUEE sendMsg;
    sendMsg.Clear();
...
void RegularVoidFieldSolo::PlayerEscapeProcess(Player* player)
{
    DamageMap::iterator damageIt;
