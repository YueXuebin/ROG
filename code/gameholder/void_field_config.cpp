...
IMPLEMENT_SINGLETON(VoidFieldConfig)

VoidFieldConfig::VoidFieldConfig()
{
    Clear();
}
...
bool VoidFieldConfig::LoadConfig(const char* path)
{
    ...
    for(TiXmlElement* xmlEle = xmlRoot->FirstChildElement(); xmlEle; xmlEle = xmlEle->NextSiblingElement())
    {
        ...
        else if(strcmp(xmlEle->Value(), "moonless_broadcast_id") == 0)
        {
            ConfigReader::Read(xmlEle, "value", m_moonless_broadcast_id);
        }
        ...
        else if(strcmp(xmlEle->Value(), "regular_moonless_run_time") == 0)
        {
            ConfigReader::Read(xmlEle, "value", m_regular_moonless_run_time);
        }
        ...
        else if(strcmp(xmlEle->Value(), "regular_moonless_chests_rate") == 0)
        {
            ConfigReader::Read(xmlEle, "value", m_moonless_chests_rate);
        }
        ...
        else if(strcmp(xmlEle->Value(), "regular_moonless_rob_rate") == 0)
        {
            ConfigReader::Read(xmlEle, "value", m_moonless_rob_rate);
        }
        else if(strcmp(xmlEle->Value(), "rare_boss_id") == 0)
        {
            ConfigReader::Read(xmlEle, "value", m_rare_boss_id);
        }
        else if(strcmp(xmlEle->Value(), "regular_moonless_reward_box") == 0)
        {
            ConfigReader::Read(xmlEle, "box", m_moonless_reward_box_num);
            ConfigReader::Read(xmlEle, "interval", m_moonless_reward_box_interval);
        }
        ...
        else if(strcmp(xmlEle->Value(), "regular_moonless_exp") == 0)
        {
            LoadRewardExpItem(xmlEle, VOID_FIELD_TYPE_MOONLESS);
        }
...
void VoidFieldConfig::LoadChestsItems(TiXmlElement* xmlEle)
{
...
//加载宝箱奖掉落概率跟随时间降低的概率
void VoidFieldConfig::LoadChestsProbability(TiXmlElement* xmlEle)
{
    for(TiXmlElement* itEle = xmlEle->FirstChildElement(); itEle; itEle = itEle->NextSiblingElement())
    {
        uint32 time = 0;
...
//得到掉落经过多长时间的掉落概率
std::map<uint32, int32>& VoidFieldConfig::GetChestsProbabilityByTime()
{
    return m_chestsProbabilityMap;
}

void VoidFieldConfig::LoadDropItems(TiXmlElement* xmlEle)
{
    RAND_REWARD_ITEM randRewardItem;
...
void VoidFieldConfig::LoadDiffLevelDropRate(TiXmlElement* xmlEle)
{
    int32 diff_level = 0;
    int32 drop_rate  = 0;
...
int32 VoidFieldConfig::GetRewardExpInterval(int32 state_type)
{
...
int32 VoidFieldConfig::GetRewardExpNum(int32 state_type)
{
    RewardExpItemMap::const_iterator iter = m_reward_expitems.find(state_type);
    if (iter == m_reward_expitems.end())
    {
...
// 获得宝箱随机物品
REWARD_ITEM* VoidFieldConfig::GetChestsRandItem(int32 num)
{
    int32 totalNum = 0;
...
REWARD_ITEM* VoidFieldConfig::GetDropRandItem(int32 num)
{
    int32 totalNum = 0;
...
int32 VoidFieldConfig::GetDiffLevelDropRate(int32 diff_level)
{
    if (diff_level <= 0)
    {
...
bool VoidFieldConfig::LoadRewardExpItem(TiXmlElement* xmlEle, int32 state_type)
{
