 ...
 *	虚空幻境
 */
#ifndef regular_void_field_h__
#define regular_void_field_h__
...
public:
    ...
    uint32  GetAddBoxTime() { return m_add_box_time; }
    void    SetAddBoxTime(uint32 val) { m_add_box_time = val; }

    bool    GetBossParFlag() {return m_boss_par_flag;}
    void    SetBossParFlag(bool val) {m_boss_par_flag = val;}

    bool    GetMoonlessParFlag() {return m_moonless_par_flag;}
    void    SetMoonlessParFlag(bool val) {m_moonless_par_flag = val;}

private:
    uint64                          m_player_id;        ///< 角色ID
    int32                           m_chests_num;       ///< 宝箱计数
    uint32                          m_add_exp_time;     ///< 下次增加exp时间
    uint32                          m_add_box_time;     ///< 下次增加box时间
    bool                            m_boss_par_flag;    ///< 参加珍兽
    bool                            m_moonless_par_flag;///< 参加月黑
public:
...
class RegularVoidField : public RegularWorld
{
public:
    RegularVoidField(RegularDoc* pDoc);
    virtual ~RegularVoidField();
    ...
    void            ResetAddBoxTime();                                          ///< 重置玩家增加宝箱时间

    void            TriggerTaskEvent(uint32 taskEventId);

    VoidFieldPlayerRecord*  GetRecord(Player* player);                          ///< 获得记录

    Player*         GetPlayerByUnitID(uint32 unitID);                           ///< 根据玩家或宠物的ID获取玩家

    void            KillAllMonsters();                                          ///< 杀死所有怪物，珍兽和月黑使用
    void            StopRefreshMonsters();                                      ///< 停止刷怪，珍兽和月黑使用
    void            RecoverRefreshMonsters();                                   ///< 恢复刷怪，常态使用

    void            RewardPlayerBoxInMoonless();                                ///< 定时奖励玩家宝箱数,月黑状态有效
...
private:
    ...
    // =======================================================resurgence=============================================
    typedef std::map<uint64, VoidFieldPlayerRecord>     PlayerRecordMap;
    typedef std::vector<VoidFieldOpenConditon>          OpenCondList;
    ...
    //=====================================================moon black mode==========================================================
    void OnKillPlayer(uint32 deadID, uint32 killerID);
    void DeductPlayerChestsNum(Player* player);        // 玩家死亡或中途离开月黑扣除一定宝箱数
};

#endif // regular_void_field_h__
