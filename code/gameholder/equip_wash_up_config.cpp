...
void EQUIP_WASH_UP_ROOT_HEAD_STRUCT::parse(TiXmlElement* xmlRoot)
{
    if (!xmlRoot) return;
...
void EQUIP_WASH_UP_ROOT_STRUCT::parse(TiXmlElement* xmlRoot)
{
    if (!xmlRoot) return;
    for(TiXmlElement* childEle = xmlRoot->FirstChildElement(); childEle; childEle = childEle->NextSiblingElement())
    {
        ...
        {
            EQUIP_WASH_UP_ROOT_HEAD_STRUCT StructTplt;
            StructTplt.clear();
...
IMPLEMENT_SINGLETON(EQUIP_WASH_UP_ENTRY)

bool EQUIP_WASH_UP_ENTRY::LoadConfig(const char* path)
{
    char file_path[SD_MAX_PATH];
    SDSprintf(file_path, "%s%s", path, "equip_wash_up.xml");
