...
extern std::string g_current_function_name;
...
public:
	LuaCallBase(lua_State *luaState, const std::string& functionName)
	{
        ...
        g_current_function_name = functionName;
	}
...
class  LuaCall
	: public Duo<TR, class LuaCall<T1, T2, T3, T4, NullT> > 
	, public LuaCallBase
{
public:
	...
	TR call(T1 a1, T2 a2, T3 a3, T4 a4)
	{
		...
		{
			CnError("lua function %s error with 4 args\n", g_current_function_name.c_str());
		}
...
public:
	...
	TR call(T1 a1, T2 a2, T3 a3)
	{
		...
		{
			CnError("lua function %s error with 3 args\n", g_current_function_name.c_str());
		}
...
public:
	...
	TR call(T1 a1, T2 a2)
	{
		...
		{
			CnError("lua function %s error with 2 args\n", g_current_function_name.c_str());
		}
...
public:
	...
	TR call(T1 a1)
	{
		...
		{
			CnError("lua function %s error with 1 args\n", g_current_function_name.c_str());
		}
...
public:
	...
	TR call(void)
	{
		...
		{
			CnError("lua function %s error with 0 args\n", g_current_function_name.c_str());
		}
...
public:
	...
	void call(void)
	{
		...
		{
			CnError("lua function %s error with 0 args no return\n", g_current_function_name.c_str());
		}
	}
};
