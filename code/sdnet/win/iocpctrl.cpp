...
bool CIocpCtrl::Init()
{
	...
	{
		CRITICAL("CIocpCtrl::Init, CreateIoCompletionPort failed, Error %d \n", ::WSAGetLastError()); 
		return false;
	}
	...
	m_WorkerArray = CnNew HANDLE[m_nNumberOfWorkers];
	...
	for (int32 j = 0; j < m_nNumberOfWorkers; j++) 
	{
		...
		if (NULL == m_WorkerArray[j]) 
		{
			...
			this->Uninit();
			CRITICAL("CIocpCtrl::Init, Create Worker thread failed, Close Handler\n"); 
			return false;
		}
	...
	while(true)
	{
		...
		if(pstPerHandleData->bListen)
		{
			...
			if(NULL != poListener &&  NULL != pstPerIoData)
			{
				...
				//printf("Accpet Count:%d \n", InterlockedIncrement((int32*)&m_acceptCount) ); 
		...
		else 
		{
			...
			{
				INFO("[%s:%d]CCPSock connID=%d error %d, close it", 
				...
				case IOCP_RECV:
					{
						...
						{
							INFO("[%s:%d]CCPSock connID=%d error %d, close it, socket :%d ", 
...
bool CIocpCtrl::AssociateWithIocp(SOCKET hSock, SPerHandleData* pstData)
{
	...
	{
		WARN("CIocpCtrl::AssociateWithIocp, failed, errno %d", WSAGetLastError());
		return false;
	}
